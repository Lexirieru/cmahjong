...
describe("SettlementService", () => {
  ...
  it("kumpulkan 4 tanda tangan lalu submit settle kooperatif", async () => {
  ...
  it("tolak tanda tangan dari non-pemain", async () => {
    const chain = makeChain();
    const svc = new SettlementService(chain);
    svc.open(GAME, ranking, ranking);
  ...
  it("tanda tangan duplikat tidak menggandakan (butuh 4 berbeda)", async () => {
    const chain = makeChain();
    const svc = new SettlementService(chain);
    svc.open(GAME, ranking, ranking);
    await svc.addSignature(GAME, await sigOf(players[0]));
    await svc.addSignature(GAME, await sigOf(players[0])); // ulang
  ...
  it("status failed bila submit on-chain gagal", async () => {
    const chain = makeChain();
