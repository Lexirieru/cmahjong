...
export interface ScoreInput {
  /** counts 34-elemen untuk SELURUH 14 ubin (termasuk ubin penyelesai & meld terbuka) */
  counts: number[];
  /** meld terbuka hasil call (opsional) */
  openMelds?: Meld[];
  ctx: WinContext;
...
function countDora(counts: number[], indicators: number[] | undefined, aka: number): number {
  let n = aka ?? 0;
  if (indicators) {
...
function scoreStandard(d: Decomposition, input: ScoreInput): ScoreResult {
  const { ctx } = input;
  ...
  han += countDora(input.counts, input.doraIndicators, input.aka ?? 0);
  if (ctx.riichi || ctx.doubleRiichi) han += countDora(input.counts, input.uraIndicators, 0);
...
export function scoreHand(input: ScoreInput): ScoreResult {
  ...
  if (agari.chiitoitsu) {
    ...
    if (yaku.some((y) => y.yakuman)) {
      ...
      if (input.ctx.riichi || input.ctx.doubleRiichi) {
        han += countDora(input.counts, input.uraIndicators, 0);
      }
  ...
  for (const d of agari.standard) {
    const r = scoreStandard(d, input);
    if (r.agari) candidates.push(r);
  }

  if (candidates.length === 0) return NO_WIN; // menang bentuk, tapi tanpa yaku
